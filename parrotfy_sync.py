...
def pick_rows(ws_data) -> Tuple[List[List[str]], List[int]]:
    ...
    Lee columnas: fecha_correo | sku | un_recibidas | (opcional) parrotfy_enviado
    Envia solo un_recibidas > 0 y no marcadas como enviados.
    """
...
def mark_sent(ws_data, row_indexes: List[int]):
    ...
    ws_data.batch_update(updates)

# --------------------------- Builder de texto a pegar --------------
def build_import_text(pending_rows: List[List[str]], price_map: Dict[str,float]) -> Tuple[str, List[str]]:
    """
    Construye el bloque 'SKU[TAB]CANTIDAD[TAB]PRECIO' por línea.
    Si falta precio y STRICT_PRICES=True: lo omite y devuelve la lista de faltantes.
    """
    lines = []
    missing = []
    for sku, qty in pending_rows:
        ...
        if price is None:
            if STRICT_PRICES:
                ...
                continue
            price = 0.0
        lines.append(f"{sku}\t{qty}\t{price}")
    return "\n".join(lines), missing

# --------------------------- Playwright / Parrotfy -----------------
def run_parrotfy_import(import_text: str):
    """
    Entra a /inventory_movement_groups/new, configura:
      - Referencia: Otro
      - Bodega: KW
      - Centro de negocio: Marketing
    Abre 'Importar lista', pega texto y confirma.
    *** Nota: los selectores pueden requerir ajustes leves según el DOM real. ***
    """
    with sync_playwright() as p:
        ...
        # 4) Pegar líneas y confirmar
        page.fill("textarea", import_text)
        page.click('button:has-text("IMPORTAR"), button:has-text("Importar")')

        # 5) Crear
        page.click('button:has-text("CREAR"), button:has-text("Crear")')
...
def main():
    ...
    if missing:
        ...
        for s in missing[:20]:
            print("   -", s)
        if STRICT_PRICES:
            print("[SYNC] Modo estricto: no se envían filas sin precio. Se continúa con las que sí tienen.")
